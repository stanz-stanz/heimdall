...
class ScanFunctionSpec:
    """Declared spec for a Valdí-tracked scan function.

    ``helper_function`` — name of an internal helper the wrapper delegates to.
    Invariant: the helper MUST be a module-level attribute of the same
    ``module`` that exports ``function``. The runtime validator
    (``_validate_approval_tokens`` in ``src/prospecting/scanners/registry.py``)
    re-hashes the helper from that module and fails boot on mismatch,
    non-callables, lambdas, or unsourceable builtins. If you need a
    cross-module helper, extend the approvals.json schema with
    ``helper_module`` first — don't relax the invariant.
    """
    scan_type_id: str
    module: str
    function: str
...
SCAN_FUNCTIONS: list[ScanFunctionSpec] = [
    ...
    ScanFunctionSpec(
        ...
        helper_function="query_crt_sh_single",
    ),
    ScanFunctionSpec(
...
def build_regeneration_plan(now_iso: str, log_rel_path: str) -> dict:
    ...
    return {
        "new_approvals_json": {
            "_schema_note": (
                "Generated by scripts/valdi/regenerate_approvals.py. "
                "Do not hand-edit. Entries with helper_hash carry an "
                "invariant: the helper_function must be a module-level "
                "attribute of the same module as the registered function. "
                "The runtime validator fails boot on mismatch."
            ),
