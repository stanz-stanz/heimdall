...
alias heimdall-export="cd $HEIMDALL_DIR && docker compose -f $COMPOSE_FILE run --rm --no-deps -v $HEIMDALL_DIR/data/input:/data/input:ro -v $HEIMDALL_DIR/data/output:/data/output --entrypoint sh worker -c 'PYTHONPATH=/app python3 scripts/export_results.py --results-dir /data/results --output-dir /data/output --cvr-file /data/input/CVR-extract.xlsx'"
alias heimdall-analyze="cd $HEIMDALL_DIR && docker compose -f $COMPOSE_FILE run --rm --no-deps -v $HEIMDALL_DIR/data/output:/data/output:ro --entrypoint sh worker -c 'PYTHONPATH=/app python3 scripts/analyze_pipeline.py --results-dir /data/results'"
alias heimdall-deep="cd $HEIMDALL_DIR && docker compose -f $COMPOSE_FILE run --rm --no-deps -v $HEIMDALL_DIR/data/output:/data/output:ro --entrypoint sh worker -c 'PYTHONPATH=/app python3 scripts/analyze_pipeline.py --results-dir /data/results --deep'"
