...
alias heimdall-export="cd $HEIMDALL_DIR && docker compose -f $COMPOSE_FILE run --rm --no-deps -e PYTHONPATH=/app -v $HEIMDALL_DIR/data/input:/data/input:ro -v $HEIMDALL_DIR/data/output:/data/output --entrypoint python3 worker scripts/export_results.py --results-dir /data/results --output-dir /data/output --cvr-file /data/input/CVR-extract.xlsx"
alias heimdall-analyze="cd $HEIMDALL_DIR && docker compose -f $COMPOSE_FILE run --rm --no-deps -e PYTHONPATH=/app -v $HEIMDALL_DIR/data/output:/data/output:ro --entrypoint python3 worker scripts/analyze_pipeline.py --results-dir /data/results"
alias heimdall-deep="cd $HEIMDALL_DIR && docker compose -f $COMPOSE_FILE run --rm --no-deps -e PYTHONPATH=/app -v $HEIMDALL_DIR/data/output:/data/output:ro --entrypoint python3 worker scripts/analyze_pipeline.py --results-dir /data/results --deep"
