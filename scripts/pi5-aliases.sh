...
alias heimdall-pipeline="docker compose -f $COMPOSE_FILE run --rm worker python -m pytest tests/test_docker_smoke.py -v && docker compose -f $COMPOSE_FILE exec -T redis redis-cli DEL queue:scan queue:enrichment queue:wpscan && docker compose -f $COMPOSE_FILE run --rm scheduler"
