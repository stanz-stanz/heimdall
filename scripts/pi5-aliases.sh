...
alias heimdall-pipeline="docker compose -f $COMPOSE_FILE exec -T redis redis-cli DEL queue:scan queue:enrichment queue:wpscan && docker compose -f $COMPOSE_FILE run --rm scheduler"
