...
alias heimdall-deploy="cd $HEIMDALL_DIR && git pull && docker compose -f $COMPOSE_FILE build --no-cache worker && docker compose -f $COMPOSE_FILE build --no-cache api ct-collector scheduler && docker compose -f $COMPOSE_FILE -f $COMPOSE_MON up -d --remove-orphans"
