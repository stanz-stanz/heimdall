...
alias heimdall-flush="docker compose -f $COMPOSE_FILE exec -T redis redis-cli DEL queue:scan queue:enrichment queue:wpscan enrichment:completed enrichment:total"
alias heimdall-pipeline="docker compose -f $COMPOSE_FILE run --rm --entrypoint bash -v $HEIMDALL_DIR/scripts/docker-smoke.sh:/tmp/smoke.sh:ro worker /tmp/smoke.sh && docker compose -f $COMPOSE_FILE stop scheduler 2>/dev/null; docker compose -f $COMPOSE_FILE exec -T redis redis-cli DEL queue:scan queue:enrichment queue:wpscan enrichment:completed enrichment:total && docker compose -f $COMPOSE_FILE run --rm scheduler"
