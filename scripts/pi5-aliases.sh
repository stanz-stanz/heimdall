...
alias heimdall-deploy="cd $HEIMDALL_DIR && git pull && docker compose -f $COMPOSE_FILE build worker && docker compose -f $COMPOSE_FILE build api ct-collector scheduler && docker compose -f $COMPOSE_FILE -f $COMPOSE_MON up -d --force-recreate --remove-orphans"
