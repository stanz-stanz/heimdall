...
alias heimdall-pipeline="docker compose -f $COMPOSE_FILE run --rm --entrypoint python worker -m pytest tests/test_docker_smoke.py -v && docker compose -f $COMPOSE_FILE exec -T redis redis-cli DEL queue:scan queue:enrichment queue:wpscan && docker compose -f $COMPOSE_FILE run --rm scheduler"
alias heimdall-audit="cd $HEIMDALL_DIR && python3 scripts/audit.py"
alias heimdall-smoke="cd $HEIMDALL_DIR && bash scripts/docker-smoke.sh"
