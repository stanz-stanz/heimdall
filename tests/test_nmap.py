...
from src.prospecting.scanners.nmap import (
...
class TestParseNmapXml:
    ...
    def test_multi_port(self):
        result = parse_nmap_xml(NMAP_XML_MULTI_PORT, "example.dk")
    ...
    def test_no_open_ports(self):
        result = parse_nmap_xml(NMAP_XML_NO_OPEN, "example.dk")
        assert result["port_count"] == 0
        assert result["open_ports"] == []

    def test_empty_host(self):
        result = parse_nmap_xml(NMAP_XML_EMPTY, "example.dk")
        assert result["port_count"] == 0
        assert result["open_ports"] == []

    def test_malformed_xml(self):
        result = parse_nmap_xml("not xml at all", "example.dk")
        assert result["port_count"] == 0
        assert result["open_ports"] == []

    def test_partial_service_attributes(self):
        result = parse_nmap_xml(NMAP_XML_PARTIAL_SERVICE, "example.dk")
...
class TestNmapPortsToFindings:
    ...
    def test_critical_port_redis(self):
        ...
        findings = nmap_ports_to_findings(ports)
    ...
    def test_empty_list(self):
        findings = nmap_ports_to_findings([])
        assert findings == []
...
class TestRunNmap:
    ...
    def test_invalid_domain_skipped(self):
        with patch("src.prospecting.scanners.nmap.shutil.which", return_value="/usr/bin/nmap"):
            result = run_nmap(["not a valid domain!"])
            assert result == {}
