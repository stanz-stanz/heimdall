...
**Status (2026-04-28 night):** 1,482 unit tests passing on `feat/stage-a-foundation`, coverage above the 65% floor. Sentinel onboarding backend (signup-token magic-link, `activate_watchman_trial`, Telegram `/start <token>`, `src/db/conversion.py` + `src/db/retention.py` + `src/db/onboarding.py`, `src/retention/` execution cron, trial-expiry scanner + reconciler, Valdí-cleared §263 evidence preservation) fully landed in the prior `feat/sentinel-onboarding` cycle. The active branch advances Stage A's auth-plane carve per `docs/architecture/stage-a-implementation-spec.md`. Slices shipped: 1, 2, 3a–3e, 3f, **3g (d)(e)** (commit `f3f95da` — handler-level WS auth on `/console/ws` + `/console/demo/ws/{scan_id}` via `_authenticate_ws` helper + `_build_pseudo_request` adapter; new `tests/test_console_ws_auth.py` with 15 cases covering close-code, audit-row pairing, pubsub-side-effect deltas, and `last_seen_at` invariants; Codex-cleared), and **3g (f)** (legacy retirement per §7.10 Option B — `LegacyBasicAuthMiddleware` class + `HEIMDALL_LEGACY_BASIC_AUTH` env-flip branch deleted from `src/api/app.py`; auth-router include simplified to unconditional; legacy mount tests purged from `tests/test_session_auth.py`; master spec §9.1 noted as retired). **Remaining for slice 3g**: (a)(b)(c) SPA work — login view + whoami bootstrap state machine (`lib/auth.svelte.js`) + CSRF helper threaded through `lib/api.js` + `App.svelte` mount-time gate + `Topbar` logout button + `BootstrapEmpty` / `AllDisabled` splash views (~400 LOC across 4 new Svelte files + 3 edits, untestable until slice 3g.5). **Slice 3g.5 hard-gates production deploy** per §7.11 Option B: SPA Vitest harness + auth-flow tests must land green before the bundle merges to `main` / pushes to `prod` (the operator console SPA at `src/api/frontend/` has zero Vitest setup today). Audit-helper relocation prep landed as commit `146ab53`. **Rollback for slice 3g** is `git revert <merge-sha>` — the env-flip lever was retired with (f). **Pilot launch blocked by SIRI approval, not by code.** Next critical-path after slice 3g + 3g.5: A.5 (`Permission` enum + `require_permission` decorator + X-Request-ID middleware + `config_changes` triggers + `/console/config/history`) → V2 onboarding view.
