...
**Status (2026-04-28):** 1,482 unit tests passing on `feat/stage-a-foundation`, coverage above the 65% floor. Sentinel onboarding backend (signup-token magic-link, `activate_watchman_trial`, Telegram `/start <token>`, `src/db/conversion.py` + `src/db/retention.py` + `src/db/onboarding.py`, `src/retention/` execution cron, trial-expiry scanner + reconciler, Valdí-cleared §263 evidence preservation) fully landed in the prior `feat/sentinel-onboarding` cycle. The active branch advances Stage A's auth-plane carve per `docs/architecture/stage-a-implementation-spec.md`. Slices shipped: 1 (decision-log + spec hardening), 2 (argon2-cffi + `_seed_operator_zero`), 3a (session ticket lifecycle), 3b (`console.audit_log` writer), 3c (per-IP login rate limiter), 3d (`SessionAuthMiddleware` HTTP-only ASGI module + tests), 3e (auth router `/console/auth/{login,logout,whoami}` + harmonised response shape + middleware disabled-operator audit hook), 3f (`LegacyBasicAuthMiddleware` rename + `HEIMDALL_LEGACY_BASIC_AUTH=1` env gate + `SessionAuthMiddleware` mounted by default in `create_app` + auth-router include skipped under legacy mode + `git mv tests/test_console_auth.py tests/test_session_auth.py` with cookie-flow rewrite + minimal `tests/_console_auth_helpers.py` so non-auth console tests acquire a real session + forward-looking loop-break edits in `src/api/frontend/src/lib/api.js` and `src/api/frontend/src/lib/ws.svelte.js`). **Transitional state — the very next slice closes it atomically:** the Svelte SPA has no login form, `lib/api.js` does not send `X-CSRF-Token`, and `/console/ws` (`src/api/console.py:811-814`) does `await websocket.accept()` immediately with no cookie check. With `/app/*` still protected by `SessionAuthMiddleware`, the default-mode browser path returns a static 401 page on `/app/` (no SPA load, no leak); operators needing the UI flip `HEIMDALL_LEGACY_BASIC_AUTH=1` on the Pi5 (rollback runbook §9.1) for legacy Basic Auth access. Next slice (locked scope, ships atomically): SPA login view + `whoami` bootstrap probe + cookie-aware fetch + `X-CSRF-Token` helper threaded through `lib/api.js` mutation helpers + handler-level WS auth in `src/api/console.py` (cookie sniff → sha256 → validate → close 4401) + new `tests/test_console_ws_auth.py`. See the slice 3f decision-log entry for the full locked scope and the audit-trail of the option-1 / option-2 / WS-leak deliberation chain. `main` still on the 2026-04-18 hardening baseline; the active feature branch is far ahead. **Pilot launch blocked by SIRI approval, not by code.** Next critical-path after the SPA login + WS auth slice: A.5 (`Permission` enum + `require_permission` decorator + X-Request-ID middleware + `config_changes` triggers + `/console/config/history`) → V2 onboarding view.
