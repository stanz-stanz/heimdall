...
**Pi5 is PROD. Macbook is DEV.** Develop locally (`make dev-up` / `make dev-smoke`, see `docs/development.md`). Deploy: `main` → dev-smoke green → fast-forward `prod` → `HEIMDALL_APPROVED=1 git push origin prod` → SSH Pi5 `heimdall-deploy` (see `docs/runbook-prod-deploy.md`). Business phase: **pre-pilot, blocked by SIRI approval.** Recently merged: PR #46 (operator console V1 trial-expiring + V6 retention queue tabs, 2026-04-27 16:09 UTC) — V1+V6 tabs render but are empty in DEV until the DRYRUN-CONSOLE seed plan ships. Active feature work: PR #47 (`feat/dev-prod-bind-mount-separation` — finishes M37 by parameterising the four host bind-mounts so DEV reads `data/dev/*` fixture data; also bundles operator-console pipeline-progress emission + worker healthcheck heartbeat thread + dev subfinder tuning so the dev pipeline completes in ~30s with a smoothly-moving progress bar). Operator-console architectural reframe around bounded contexts (Tenant Identity / Findings Lifecycle / Onboarding / Subscription & Billing / Retention / Live Operations + Operator Identity & Audit cross-cutting; `industries` resolved to Tenant Identity 2026-04-27) is queued — four Federico decisions outstanding before Stage A foundation lands; see `docs/decisions/log.md` 2026-04-27 entry.
...
| `src/api/` | Results API + operator console (FastAPI + Svelte 5 SPA at `/app`). Router: `src/api/console.py` (9 REST endpoints incl. `/console/briefs/list` + WebSocket `/console/ws`). Signup router: `src/api/signup.py` (`POST /signup/validate` — read-only magic-link check; calls `src.db.signup.get_signup_token`, never mutates state; Origin allowlist via `SIGNUP_ALLOWED_ORIGINS`, 503 if `TELEGRAM_BOT_USERNAME` env unset). Frontend: `src/api/frontend/` (Vite build → `src/api/static/dist/`). Views: Dashboard, Pipeline, Campaigns, Prospects, Briefs, Clients, Logs, **Live Demo** (`src/api/frontend/src/views/LiveDemo.svelte` — brief selector with prefix search + 24/page pagination, WebSocket-driven scan replay via `demo_orchestrator.run_demo_replay`, streamed findings with typewriter effect sorted by severity desc, spotlight Assessment Complete summary), Settings. Hash-based router at `src/api/frontend/src/lib/router.svelte.js` (`#/view?k=v` — `router.params` bag for deep-links; persists across refresh). Dashboard stat cards are clickable and navigate to their populated list view. **Theme:** light + dark, toggle in Topbar (`ThemeToggle.svelte` + `src/api/frontend/src/lib/theme.svelte.js`), defaults to `prefers-color-scheme`, override persists in `localStorage['heimdall.theme']`. Design system at `docs/design/design-system.md` v1.3 (warm-only severity in both themes, 11 type utility classes incl. `.t-help` for explanatory prose). |
...
## Build Priority: Sentinel Onboarding Backend Complete → SvelteKit + Operator Console

**Status (2026-04-25):** 1,201 unit tests passing on `feat/sentinel-onboarding`, coverage above the 65% floor. Sentinel onboarding backend fully landed: signup-token magic-link, `activate_watchman_trial` activation transition, Telegram `/start <token>` handler, `src/db/conversion.py` + `src/db/retention.py` + `src/db/onboarding.py` helpers, `src/retention/` execution cron (claim-lock, action handlers, scheduler timer), trial-expiry scanner + reconciler, Valdí-cleared §263 evidence preservation in anonymise. Pre-commit Codex review hook + agent-routing discipline (graph-before-Grep, Codex-before-commit) shipped this session. `main` still on the 2026-04-18 hardening baseline; the active feature branch is far ahead. **Pilot launch blocked by SIRI approval, not by code.** Next critical-path: SvelteKit signup site + operator console views V1–V6 + Message 0 magic-link email sender.
