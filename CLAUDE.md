...
| `src/outreach/` | Outreach module — three batch commands: `promote` (filter briefs → prospects table), `interpret` (Claude API on filtered subset with `--min-severity` flag), `send` (compose for delivery). Entry: `python -m src.outreach promote\|interpret\|send --campaign MMYY-industry`. |
