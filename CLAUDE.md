...
| `Makefile` | Mac dev ergonomics — 17 targets: `dev-up`, `dev-down`, `dev-nuke`, `dev-logs`, `dev-ps`, `dev-shell`, `dev-seed`, `dev-seed-check`, `dev-pytest`, `dev-pytest-integration`, `dev-smoke`, `compose-lint`, `prod-render`, `dev-render`, `dev-build`, `check-env`, `help`. Every target pins `-p heimdall_dev` and refuses to run without `infra/compose/.env.dev`. |
