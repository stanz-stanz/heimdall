...
| `docs/architecture/stage-a-implementation-spec.md` | **Stage A master spec** — auth-plane carve (1763 lines). Cite by section number from per-slice specs; never restate contracts. §3 (auth router), §4 (security), §5 (WS auth), §6 (router layout), §7 (audit), §8 (test plan), §9 (rollback), §11 (out of scope). |
| `docs/architecture/stage-a-slice-3g-spec.md` | **Stage A slice 3g spec — LOCKED** (commit `3ea2fcf`). SPA login + whoami bootstrap + CSRF helper + handler-level WS auth on `/console/ws` + `/console/demo/ws/{scan_id}` + new `tests/test_console_ws_auth.py` + retire `LegacyBasicAuthMiddleware` per §7.10 Option B. Slice 3g.5 hard-gates production deploy on Vitest SPA tests per §7.11 Option B. All eleven §7 decisions locked 2026-04-28 evening. |
...
**Status (2026-04-28 late evening):** 1,482 unit tests passing on `feat/stage-a-foundation`, coverage above the 65% floor. Sentinel onboarding backend (signup-token magic-link, `activate_watchman_trial`, Telegram `/start <token>`, `src/db/conversion.py` + `src/db/retention.py` + `src/db/onboarding.py`, `src/retention/` execution cron, trial-expiry scanner + reconciler, Valdí-cleared §263 evidence preservation) fully landed in the prior `feat/sentinel-onboarding` cycle. The active branch advances Stage A's auth-plane carve per `docs/architecture/stage-a-implementation-spec.md`. Slices shipped: 1, 2, 3a–3e, **3f** (commit `d0bc063` — `LegacyBasicAuthMiddleware` rename + `HEIMDALL_LEGACY_BASIC_AUTH=1` env gate + `SessionAuthMiddleware` mounted by default in `create_app` + `tests/_console_auth_helpers.py` + forward-looking loop-break edits in `lib/api.js` and `lib/ws.svelte.js` + `git mv test_console_auth.py → test_session_auth.py`). **Slice 3g spec LOCKED** at `docs/architecture/stage-a-slice-3g-spec.md` (commit `3ea2fcf`) — atomic six-component scope (a–f) covering SPA login + whoami bootstrap + CSRF helper + handler-level WS auth on `/console/ws` + `/console/demo/ws/{scan_id}` + new `tests/test_console_ws_auth.py` + retiring `LegacyBasicAuthMiddleware` and `HEIMDALL_LEGACY_BASIC_AUTH` per §7.10 Option B. **Slice 3g.5 hard-gates production deploy** per §7.11 Option B: SPA Vitest harness + auth-flow tests must land green before the bundle merges to `main` / pushes to `prod` (the operator console SPA at `src/api/frontend/` has zero Vitest setup today). Audit-helper relocation prep landed as commit `146ab53` (`maybe_write_disabled_operator_audit` moved to `src/api/auth/audit.py` for HTTP-middleware + WS-handler symmetry per §7.9). **Remaining for slice 3g** (next session): (d)(e) WS handler auth + new test file (~240 LOC backend, fully testable), (a)(b)(c) SPA work (~400 LOC across 4 new Svelte files + 3 edits, untestable until 3g.5), (f) legacy retirement (work-in-flight stashed at `stash@{0}`). **Transitional state until slice 3g ships:** default-mode browser hits `/app/` → static 401 (SPA has no login form yet); for ops UI access the legacy `HEIMDALL_LEGACY_BASIC_AUTH=1` flag remains the bridge until (f) retires it atomically with the rest of slice 3g; `/console/ws` is still unauthenticated at the handler layer — the data-leak surface that slice 3g (d) closes. `main` still on the 2026-04-18 hardening baseline; the active feature branch is far ahead. **Pilot launch blocked by SIRI approval, not by code.** Next critical-path after slice 3g + 3g.5: A.5 (`Permission` enum + `require_permission` decorator + X-Request-ID middleware + `config_changes` triggers + `/console/config/history`) → V2 onboarding view.
