...
| `apps/signup/` | SvelteKit signup site (slice 1 dev-ready 2026-04-25). Independent of `src/api/frontend/` — own `package.json`, `node_modules`, `vite.config.js`. Adapter-static; Vite dev server `:5173` proxies `/api/*` → host `:8001` (dev FastAPI host port). Six routes: `/`, `/pricing`, `/legal/{privacy,terms,dpa}`, `/signup/start?t=<token>` (magic-link landing — calls `POST /api/signup/validate`, renders Telegram CTA + QR, strips token from URL via `history.replaceState(history.state, ...)` to preserve SvelteKit router state). Tokens copied verbatim from `src/api/frontend/src/styles/tokens.css`. i18n in `src/lib/i18n.js` (`t(key)`, EN-default + EN-fallback when DA missing). API wrapper in `src/lib/api.js`. Pricing reads `src/lib/pricing.json` — single Sentinel plan, 30-day Watchman trial as a feature. Vitest 9/9 (`make signup-test`). Browser-eyeball QA still needed; visual tune-up deferred. |
