...
**Pi5 is PROD. Macbook is DEV.** Develop locally (`make dev-up` / `make dev-smoke`, see `docs/development.md`). Deploy: `main` → dev-smoke green → fast-forward `prod` → `HEIMDALL_APPROVED=1 git push origin prod` → SSH Pi5 `heimdall-deploy` (see `docs/runbook-prod-deploy.md`). Business phase: **pre-pilot, blocked by SIRI approval.** Active feature work: PR #46 (`feat/operator-console-v1-v6` — operator console V1 trial-expiring + V6 retention queue tabs) and PR #47 (`feat/dev-prod-bind-mount-separation` — finishes M37 by parameterising the four host bind-mounts so DEV reads `data/dev/*` fixture data instead of prod paths). Operator-console architectural reframe around bounded contexts (Tenant Identity / Findings Lifecycle / Onboarding / Subscription & Billing / Retention / Live Operations + Operator Identity & Audit cross-cutting) is queued — five Federico decisions outstanding before Stage A foundation lands; see `docs/decisions/log.md` 2026-04-27 entry.
