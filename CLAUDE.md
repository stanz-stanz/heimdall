...
| `src/api/` | Results API + operator console (FastAPI + Svelte 5 SPA at `/app`). Router: `src/api/console.py` (9 REST endpoints incl. `/console/briefs/list` + WebSocket `/console/ws`). Frontend: `src/api/frontend/` (Vite build → `src/api/static/dist/`). Views: Dashboard, Pipeline, Campaigns, Prospects, Briefs, Clients, Logs, Settings. Hash-based router at `src/api/frontend/src/lib/router.svelte.js` (`#/view?k=v` — `router.params` bag for deep-links; persists across refresh). Dashboard stat cards are clickable and navigate to their populated list view. Design system at `docs/design/design-system.md` v1.2 (warm-only severity, 11 type utility classes incl. `.t-help` for explanatory prose). |
