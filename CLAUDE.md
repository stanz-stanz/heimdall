...
| `src/core/secrets.py` | Secrets helper — `get_secret(file_name, env_name, default)` reads `/run/secrets/<file_name>` (compose secret mount) first, falls back to `os.environ[env_name]`. Warns once per secret if env fallback fires inside a container (misconfig signal). Keeps `patch.dict(os.environ, ...)` tests passing unchanged. |
| `scripts/migrate_env_to_secrets.sh` | Idempotent split of `.env` credentials into per-secret files for compose `secrets:` mounts. Used on Pi5 once post-PR-D; auto-invoked by `make dev-secrets` on Mac. Backs up the env file as `.env.pre-secrets`, strips migrated lines, chmod 600 on outputs. |
| `.github/workflows/publish-images.yml` | Publishes 5 `linux/arm64` images to GHCR on every push to main. Matrix-parallel via buildx + QEMU on ubuntu-latest. Three tags per service: `:<full-sha>`, `:<short-sha>`, `:main`. Provenance + SBOM. Per-service GHA cache scope to avoid thrashing. Cold worker: ~15-25 min. Warm: ~5-8 min. |
| `.github/workflows/prune-ghcr.yml` | Monthly retention policy for the 5 `heimdall-*` GHCR packages. Keeps the most recent 30 tagged versions per service. Runs 03:00 UTC on the 1st. |
...
| `infra/compose/docker-compose.dev.yml` | Dev stack overlay for Mac (sibling to `docker-compose.yml`). Publishes Redis 6379 on host, pins worker replicas=1, shifts api to port 8001, gates dozzle behind `profiles: ["tools"]`. Loaded via `make dev-up` with project `-p heimdall_dev`. |
| `Makefile` | Mac dev ergonomics. Lifecycle: `dev-up`, `dev-down`, `dev-nuke`, `dev-logs`, `dev-ps`, `dev-shell`, `dev-build`, `dev-secrets`, `check-env`. Data: `dev-seed`, `dev-seed-check`. Tests: `dev-pytest`, `dev-pytest-integration`, `dev-smoke`. Ops verification: `dev-ops-smoke` (backup.sh regression guard + project-name resolution + /run/secrets populated + no env-var fallback), `dev-cert-dry-run` (Sentinel cert-change path end-to-end against synthetic target). Lint/diff: `compose-lint`, `prod-render`, `dev-render`. Every target pins `-p heimdall_dev` and refuses to run without `infra/compose/.env.dev`. |
...
| `scripts/dev/cert_change_dry_run.py` | Sentinel cert-change alert dry run. Runs inside dev delivery container via `make dev-cert-dry-run` (docker cp + docker exec). Seeds a fabricated stale cert snapshot under CVR prefix `DRYRUN-`, drives real `poll_and_diff_client` against `heimdall.dk`, asserts publisher + composer + subscriber path end-to-end. No Telegram send (synthetic client `telegram_chat_id=NULL`). Idempotent; `finally`-block cleanup on `DRYRUN-*` rows. Wall-clock ~10s. |
| `config/ct_dry_run.json` | Config for `scripts/dev/cert_change_dry_run.py` — synthetic CVR prefix, target domain, fake stale cert fields, subscriber wait window. Separate from `config/synthetic_targets.json` (twin consent-bypass framework; CT log data is consent-irrelevant public info). |
| `scripts/dev/interpret_dry_run.py` | M33 operational verification. Dev-stack dry run for the Finding Interpreter path. Runs inside dev delivery container via `make dev-interpret-dry-run` (docker cp + docker exec). Seeds a synthetic Watchman client + domain + brief under CVR prefix `DRYRUN-INT-`, publishes `client-scan-complete` on Redis, observes `console:logs`. Two modes: `--mode=observe` (default, free — NULL chat_id, runner early-returns before Claude) and `--mode=send-to-operator` (~$0.02 real Claude API call — full interpret + compose + Telegram send to `TELEGRAM_OPERATOR_CHAT_ID`). Cost guard refuses under `CI=true`/`GITHUB_ACTIONS=true` unless `HEIMDALL_ALLOW_PAID_DRYRUN=1`. Cleanup is mode-aware: both modes pre-clean the prefix; observe mode also post-cleans; send-to-operator skips post-cleanup so Telegram Approve/Reject callbacks resolve. |
| `config/interpret_dry_run.json` | Config for `scripts/dev/interpret_dry_run.py` — synthetic CVR, domain, plan, language, subscriber wait window. Separate from `config/ct_dry_run.json` (different pipeline path — interpret vs cert monitoring). |
...
**Status (2026-04-18):** MVP hardening Phase 1/2 shipped (PRs #23, #25). Scanner decomposed. 976 tests passing on main, coverage 70.26% (floor 65). Valdí tokens regenerated post-refactor; helper-hash enforcement now live for 3 co-located helpers (see `src/prospecting/scanners/registry.py::_validate_helper_hash`). Pi5 microSD backup operational. `.claude/hooks/` enforcing rules mechanically. Docker infra unpark complete (PRs #30-#37): dev/prod split, external volumes, multi-stage Dockerfiles, SHA-tagged images, file-backed secrets, GHCR publish + registry-backed rollback, all verified end-to-end on Pi5. All four 2026-04-16 handoff priorities closed 2026-04-17 (helper-hash gap, env-passthrough regression test, cert-change dry run, dev Telegram bot). Post-hardening cleanup sequence shipped 2026-04-18: PR #39 (`init_db` applies pending migrations + WAL checkpoint), PR #40 (ct_monitor publish-after-commit race fix + regression test), PR #41 (M33 operational verification — interpret dry run + Pi5 `heimdall-verify-secrets` alias). **M33 complete. Pilot launch blocked by SIRI approval, not by code.**
