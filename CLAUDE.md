...
**Status (2026-04-17):** MVP hardening Phase 1/2 shipped (PRs #23, #25). Scanner decomposed. 981 tests passing on main, coverage 70.26% (floor 65). Valdí tokens regenerated post-refactor; helper-hash enforcement now live for 3 co-located helpers (see `src/prospecting/scanners/registry.py::_validate_helper_hash`). Pi5 microSD backup operational. `.claude/hooks/` enforcing rules mechanically. Docker infra unpark complete (PRs #30-#37): dev/prod split, external volumes, multi-stage Dockerfiles, SHA-tagged images, file-backed secrets, GHCR publish + registry-backed rollback, all verified end-to-end on Pi5. All four 2026-04-16 handoff priorities closed 2026-04-17 (helper-hash gap, env-passthrough regression test, cert-change dry run, dev Telegram bot). **Pilot launch blocked by SIRI approval, not by code.**
