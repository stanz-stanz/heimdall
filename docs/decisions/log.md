...
**Unresolved**
- Feature branch `chore/ct-rebuild-sentinel-monitoring` not yet pushed or PR'd. Two commits: `41b885e` (Phase 1 — deletion + crt.sh SAN extraction + worker rewire, 12 files, −1,435 / +34) and `e6a0209` (Phase 2 — Sentinel build, 18 files, −131 / +1,434).
- Pi5 deploy of this branch pending: requires `CERTSPOTTER_API_KEY` in `infra/docker/.env` (sign up at `https://sslmate.com/account/ct_search_api`), `docker compose stop ct-collector ct-backfill && docker compose rm -f ct-collector ct-backfill`, rebuild worker+scheduler+api+delivery, run `docker exec docker-api-1 python -m src.db.migrate` to apply the new schema, drop `docker_ct-data` volume.
- End-to-end Sentinel monitoring smoke test pending: insert a test Sentinel client with `monitoring_enabled=1`, manually dispatch `monitor-clients` via the console, verify `client_cert_snapshots` baseline is written with zero alerts, then simulate a change event and verify Telegram delivery.
- Valdí helper-hash gap — `query_crt_sh_single` source changes are not captured by the current approval token. Low priority; consider adding helpers to the approval schema during a future Valdí hardening pass.
- No regression test asserting compose.yml passes `CERTSPOTTER_API_KEY` to the scheduler and delivery services — shipping-theater pattern risk. Consider adding a compose-lint test as a separate hardening item.
