...
- **Fork (h) — RBAC role-name vocabulary.** Spec §4.2.1 defined `ROLE_PERMISSIONS = {"operator": frozenset(Permission)}`; the seeded reality at `src/db/console_connection.py:155` and 8 test fixtures is `'owner'`. Federico locked **option b: keep `'owner'`, no data migration** 2026-05-02. Spec §4.2.1 + new §11.8 amended in commit (4) wrap-up edits — implementation reads `ROLE_PERMISSIONS = {"owner": frozenset(Permission)}`. Decorator's case-norm + whitespace-strip lookup ((`role_hint or "").lower().strip()`) absorbs typo'd seeds (`'Owner'` / `'  owner  '`) per network-security peer-review P1 #4. Future structured RBAC (table-backed roles, `'observer'` / `'editor'` entries) revisits the vocabulary — out of A.5 scope per spec §4.2.1 docstring. (Fork letter (h) used to disambiguate from spec §11.2's pre-existing Fork (b) — `config_changes` scope for file-backed settings — which was resolved 2026-05-01 = (iii) and is unrelated to this fork.)
...
- ~~Commit (4) — `scripts/verify_audit_triggers.sh` extension~~. **Shipped `10589c2` 2026-05-02 evening immediately after wave C** as a 12-line bash diff: new check #4 `SELECT COUNT(*) FROM operators WHERE role_hint IS NOT NULL AND role_hint != 'owner'` — must return 0 at deploy. Mirrors the existing NULL check shape, exits 1 on any non-canonical role_hint so a typo'd seed script ('Owner' / 'operator' / 'admin') cannot ship past the deploy gate. Smoke-tested both paths (happy: exit 0; non-canonical operator inserted: exit 1 with the right FAIL message). Bash diff only — no Codex round (hook scope is `src/**/*.py` + `tests/**/*.py`).
- ~~6 commits unpushed.~~ **Pushed 2026-05-02 evening + PR #56 opened** (`https://github.com/stanz-stanz/heimdall/pull/56`). Bundled Stage A + A.5 Pi5 cutover (decision A.5-deploy = b, spec §7.4) gated on PR review/merge + Wernblad §263 stk. 3 confirmation if the 10y horizon must lock before deploy.
...
- New connection helper `connect_clients_audited(db_path)` in `src/db/connection.py`. HeimdallConnection factory + `audit_context()` UDF registration + `foreign_keys=ON` + Row factory; NO schema reload, NO migrations. Used by the api container which intentionally cannot run `init_db()` on `clients.db` (TOCTOU between `PRAGMA table_info` and `ALTER TABLE` racing the writer containers — see `src/api/app.py:331-339`). Three-way split locked (init_db = writer containers, connect_clients_audited = api, open_readonly = immutable readers); each opener differs by exactly one axis (schema-load, migrate, UDF, mode). Fork resolved Codex-second-opinion 2026-05-02 (option (a) of four; full rationale in conversation transcript).
- Wrapper sweep at 5 API/cron call sites, all under `bind_audit_context` so the trigger row stamps actor/intent/request_id rather than landing NULL: (1) `_run_retention_action` in `src/api/console.py:622+` covers operator-driven force-run + retry; (2) inline cancel CAS at `console_retention_cancel`; (3) `console_settings_update` at `console.py:810+` with new helper `_write_settings_audit_row` per spec §4.1.8 — hand-written `clients.audit_log` row (NOT trigger-driven; audit_log is the audit destination), `action='config.file_write'`, payload carries `{old_sha256, new_sha256}`, no-op writes (sha equal) skip the row; (4) `delivery/bot.py:handle_start_command` wraps `activate_watchman_trial` with `intent='trial.activated'`, `actor_kind='system'`; (5) `retention/runner.py:tick` wraps reap, claim, dispatch (per-job `intent=f"retention.{action}"`), DRYRUN-skip, and the two failure-branch reschedules.
- Runner-introduced cron-intent vocabulary (locked, documented in spec §4.1.5): `retention.reap`, `retention.claim`, `retention.dryrun_skip`, `retention.<action>` (anonymise/purge/purge_bookkeeping/export), `retention.backoff`, `retention.terminal_fail`. All emitted with `actor_kind='system'`. Cascade DELETEs inside `purge_client` inherit the parent `retention.purge` intent — single grep reconstructs the cascade across all 5+ trigger rows per logical step.
- `purge_bookkeeping` extension in `src/retention/actions.py:600-803` implementing all 5 Valdí 2026-04-30 binding carve-outs (spec §4.1.7): (1) per-row `datetime(occurred_at) < datetime(cutoff)` filter on each audit DELETE, where cutoff = `now - 5*365 days` via `_audit_preservation_cutoff_iso()`; (2) summary `clients.audit_log` row (`action='retention.bookkeeping_purge'`, `actor_kind='system'`, `payload_json={"deleted_counts":{...}, "occurred_at_cutoff":"<iso>"}`) INSERTed BEFORE the DELETEs; (3) `data_retention_mode='hold'` short-circuit at top — early return with `{...zeros, "held": True}`; (4) `target_pk` carve-out for orphan `config_changes` (natural via `WHERE target_pk = ?`); (5) `audit_log` DELETE filters `target_type='cvr'` to exclude colliding non-CVR rows (Codex P2 follow-up — `settings_file` rows whose `target_id` is a filename could otherwise collide with a CVR string). Pre-count and DELETE predicates match byte-for-byte; `datetime()` wrap normalises away the second-vs-millisecond precision skew between `clients.audit_log` writers (second precision today) and `config_changes` / `command_audit` (millisecond).
- Cascade-bind ratified at one logical step → N trigger rows: when the runner wraps `_dispatch_action(conn, job)` with `intent='retention.purge'`, all cascade DELETEs inside `purge_client` (consent_records, signup_tokens, client_domains, retention_jobs siblings) inherit the same intent. `(intent, table_name, target_pk, op)` row is unambiguous; per-table intent split would lose the one-step grep without gaining anything (architect ratification 2026-05-02).
- Spec §4.1.5 expanded with the runner-introduced cron intents AND a known-bypass-writers block: (a) `src/client_memory/trial_expiry.py:126` flips `clients.status` watchman_active → watchman_expired and produces a NULL-actor bypass row by design — high-frequency, wrap deferred to commit (2) or (3) if operator-console UX needs the attribution; (b) operator-set `data_retention_mode='hold'` via raw SQL produces a NULL-actor breadcrumb that must NOT be silently wrapped when V2's `retention_holds` table lands (forensically interesting event). Spec §4.1.2 writer attribution for `signup_tokens` corrected: redemption writer is `src/delivery/bot.py:handle_start_command` via `activate_watchman_trial`, not `src/api/signup.py` (which is read-only validate). `retention.terminal` renamed to `retention.terminal_fail` for grep clarity vs `retention.<action>`.
- Tests: 4 new test files per plan §test-targets. `tests/test_config_changes_triggers.py` (28 cases: 24 parametrised wrapper-bound + bypass for all 12 triggers, plus 4 named for noise-skip / consent PII preservation / signup_tokens redemption / retention.force_run end-to-end). `tests/test_settings_audit_writer.py` (4 cases per spec §4.1.8; test 3 calls the helper directly because X-Request-ID middleware lands in commit (3)). 9 Valdí preservation tests in new `TestValdiAuditPreservation` class in `tests/test_retention_actions.py` (8 spec §6.3 cases + 1 Codex follow-up `test_purge_bookkeeping_skips_non_cvr_audit_log_rows`). `tests/test_db_connection.py:test_init_db_reapply_does_not_double_insert_audit_rows` locks idempotency contract. Test seed timestamps derived from `datetime.now(UTC)` at import (Codex P2 fix — absolute-string seeds would have drifted past the 5y cutoff around 2030). Adjusted 4 raw `sqlite3.connect` test-setup sites in `test_console_endpoints.py` / `test_retention_runner.py` / `test_signup_round_trip.py` / `test_api_signup_validate.py` to `connect_clients_audited` because they perform UPDATE on tier-1 tables and crashed with `no such function: audit_context` after the wrapper sweep landed. **1562 tests passing on the working tree.**
- Decision lineage chain executed per `feedback_self_review_chain`: discovery → architect (Application Architect agent, 2026-05-02 review at SHIP) → Codex (3 P2 findings, all addressed: target_type='cvr' guard, datetime() precision wrap, derived test-seed timestamps) → python-expert peer review pending → Federico ratification.

**Rejected**

- Wrapping `transition_onboarding_stage` in `src/db/conversion.py:305` inside the helper itself. Zero in-tree callers today (V2-only). Wrapping inside the DB-layer helper would lock a default intent for all future callers and conflict with D2's "wrappers at call sites, not in DB-layer helpers" principle (architect P2 finding ratified 2026-05-02). Defer to V2.
- Wrapping the no-op `subscription.created` / `subscription.cancelled` writers (`src/db/subscriptions.py`). Zero in-tree callers today. Same disposition: wrap at the V2 Betalingsservice webhook site when it lands.
- Auto-running `/codex:review` from the model. CLAUDE.md ratifies the slash command as Federico-typed; the `precommit_codex_review_guard.py` hook enforces the env-var gate at commit time. Memory file `feedback_no_assumed_agreement.md` saved 2026-05-02 to lock the rule (HARD): nothing other than explicit agreement is agreement.
- Per-table intent split for cascade DELETEs (e.g. `retention.purge.consent_records`). Considered + rejected by architect — `(intent, table_name, target_pk, op)` row is already unambiguous; per-table intents would fragment the grep for "what happened in this purge" without gaining forensic clarity.

**Unresolved**

- Commit (1) is on working tree, NOT YET COMMITTED. Federico to ratify and trigger `/codex:review` + commit + push.
- Commit (2) — RBAC decorator + Permission enum (7 values per fork (c)) + 18 HTTP route gates + WS inline gates per spec §4.2 / §6.4 — pending after commit (1) lands.
- Commit (3) — X-Request-ID middleware + WS adapter integration + `test_request_id_propagated_across_two_dbs` cross-DB correlation proof per spec §4.3 / §6.5 — pending; can land in parallel with commit (2) per spec §3.1 (no dependency).
- Pi5 cutover for the bundled Stage A + Stage A.5 deploy per spec §7.4 (decision A.5-deploy = b) — bundled with all three commits, gated on commit (2) + (3) landing first.
- Wernblad confirmation on §263 stk. 3 → 10y horizon (decision-log line 477). Affects only `purge_bookkeeping` schedule timing; carve-out shape is unchanged. No code change required if 5y stays.
- `trial.expired` bypass-row writer in `client_memory/trial_expiry.py` — wrap or leave as documented bypass. Defer decision to commit (2) or (3).

**Next-session opener**

> "Resume on `feat/stage-a-5-audit-rbac-requestid`: commit (1) wrapper sweep + Valdí extension is on the working tree (1562 passing, architect SHIP, Codex P2s addressed). Run `/codex:review`, ratify, commit + push. Then start commit (2) RBAC decorator (`Permission` enum + `require_permission` + 18 HTTP gates + 2 WS inline gates) per spec §4.2."

---

## 2026-04-28 — DRYRUN-CONSOLE verify, PR #49 mount fix, Stage A spec + slice 1

> Recovered from a stale local commit (`d6df018`, authored 2026-04-28 16:18, never pushed) on 2026-04-29 during a tree cleanup. Only the decision-log addition is preserved; the original commit's CLAUDE.md "Build Priority" edits are dropped because that section was completely rewritten during the 2026-04-29 CLAUDE.md slim (commit `350edf7`). Order in this log file does not match strict chronology — this entry covers Stage A spec authoring + slice 1 implementation, which precede the slice 3g locked-scope entry that already sits below it.

**Decided**

- Live verification of the DRYRUN-CONSOLE seed surfaced two real bugs the unit tests missed: (1) the V1 query rejected day-0 `trial_expires_at` rows once any wall-second elapsed (`CAST(julianday(...)) AS INTEGER` floors); (2) day-7 rows displayed as 6 for the same reason. Fixed by shifting the seed list to `[1, 1, ..., 7, 7, 7]` and making the verify script tolerate either the `[1, 7]` zero-skew range or the `[0, 6]` any-skew range (sort + count + distinct-day-count + bounds, not exact range). Lesson logged in `feedback_build_reusable_verify_scripts.md` (memory): no more ad-hoc `curl + python -c` verification snippets in chat — ship as committed scripts. Universal rule, not Pi5-specific. Federico has corrected the same pattern multiple times across sessions; the memory is sharpened to treat any temptation to one-line-curl as a hard stop.
- PR #49 (direct to main, merged `78d2a31`): api `client-data` mount flipped from `:ro` to RW. Latent bug shipped with PR #46 — every retention CAS UPDATE would have raised `attempt to write a readonly database`. Caught during the Stage A spec architectural review (Outcome C of the api-mount investigation). Pre-pilot status meant nobody had clicked the buttons, so it was latent, not customer-affecting. New `scripts/dev/verify_api_clients_db_write.sh` + `make dev-verify-api-write` guards the regression class.
- Stage A spec written + persisted at `docs/architecture/stage-a-implementation-spec.md`. Three architect passes (initial 1056 → security review 1595 → tightening pass 1768 → mount-fix trim 1761). All nine open decisions resolved on this date. Establishes the standing process pattern: future architect dispatches persist under `docs/architecture/` so the bridging context survives across sessions. Notable security decisions: session tokens hashed at rest (SHA-256 of the cookie; cookie value never persisted server-side); audit ownership split per Option B (`console.audit_log` in console.db owned by api for auth events; `clients.audit_log` in clients.db owned by writer containers — no false cross-DB atomicity claim); WS auth in the handler not `BaseHTTPMiddleware` (which doesn't reliably gate WS upgrades in Starlette); `/console/auth/whoami` returns four states (200 / 204 empty bootstrap / 409 all disabled / 401 unauthenticated). Notable scope-discipline decisions: rate limiting normative-integrated into the login flow (D1, per-IP, 5 fails / 15 min via Redis); `config.update` audit explicitly OUT of Stage A (D8, deferred to A.5's `config_changes` triggers); retention endpoints stay synchronous (D9, no async-202 dispatch — that was workflow redesign smuggled into an auth/identity sprint).
- Stage A slice 1 committed (`45913d4` + `7138fbc`): `console.db` schema scaffold (`operators` / `sessions` / `audit_log`), `clients.audit_log` SECTION 11 added to `client-db-schema.sql`, `init_db_console()` factory in `src/db/console_connection.py` mirroring `src/db/connection.init_db()` (WAL + FK + sync + cache_size + fail-loud schema + parent-dir create + WAL checkpoint), api lifespan wires `init_db_console()` only — NOT `init_db()` for clients.db, deliberately, to avoid a concurrent-migration race against the writer containers' `apply_pending_migrations()`. New `console-data` named volume + RW mount on api at `/data/console`; prod runbook updated with one-time `docker volume create docker_console-data` step. No behaviour change yet — api still uses the legacy `BasicAuthMiddleware` until slice 2 ships `argon2-cffi` + the operator-#0 seed. 12 new tests (covering tables / indexes / pragmas / idempotent re-init / parent-dir / fail-loud / FK CASCADE on sessions / FK RESTRICT on audit_log via session-chain AND operator-only paths / case-insensitive UNIQUE on `operators.username`). Total suite: 1343 passing, no regressions.
- Memory: `project_smb_service_outcome_positioning.md` added — detection is the engine, service outcome (monitoring + triage + remediation + peace of mind) is the commercial headline. Drove the positioning audit + edits across 4 client-facing surfaces (signup site EN+DA, pricing labels, Email 1 + DM templates, SIRI §3 thesis paragraph) on `feat/positioning-service-outcome-pass` (committed `3b5ab36`, unpushed at the time of this entry).

**Rejected**

- Async retention dispatch with HTTP 202 in Stage A. Architect's earlier draft introduced "intent → enqueue → writer outcome" semantics. Federico's framing: "Stage A scope is drifting into workflow-behavior redesign via async retention dispatch." Reverted in the tightening pass. Stage A retention endpoints stay sync; if async is the right answer for V2 / A.5, it's a separate spec.
- Including the api mount widening inside Stage A's compose surface. Extracted into PR #49 direct to main; Stage A consumes the fix rather than ships it. Mount-widening prose moved out of the spec in the trim pass.
- Codex's [4a] HIGH finding (claimed schema columns `mfa_secret` / `role` / `session_token` / `ip_address` mismatch) was a hallucinated checklist; the spec and DDL match. Disregarded.
- Operator-#0 seed in slice 1 with a placeholder hash. Codex review #3 flagged that any non-final `password_hash` row is a one-way door — once any operator row exists, the seed never re-fires, leaving bootstrap state poisoned. Deferred entirely to slice 2 alongside `argon2-cffi`. The `operators` table stays empty in slice 1; api keeps using legacy Basic Auth.

**Unresolved (at original commit time, 2026-04-28 16:18; status as recorded then — most have since landed)**

- Stage A slice 2: `argon2-cffi` dep + `src/api/auth/hashing.py` + `_seed_operator_zero()` wired into `init_db_console()` + tests. Branch ready: `feat/stage-a-foundation`, 6 commits, unpushed.
- PR #50 (`feat/dryrun-console-seed`) open at https://github.com/stanz-stanz/heimdall/pull/50, awaiting Federico's review/merge. Browser eyeball at `http://localhost:8001/app#/clients?tab=trial-expiring|retention-queue` is the last unticked item in the test plan.
- `feat/positioning-service-outcome-pass` unpushed (4 surfaces edited, single commit `3b5ab36`). Federico hasn't said push/merge yet.
- Two untracked plans in working tree (`docs/plans/robustness-acquisition-roadmap.md`, `docs/plans/toolset-broadening-plan.md`) — Federico-authored in a parallel terminal, not part of this session's work.
- `fix/api-clients-db-mount` local branch can be deleted (PR #49 merged into main). Federico to clean up if desired.
- Slice 1 Codex LOW findings deferred: [2a] uvicorn-multi-worker race on `init_db_console()` (IF NOT EXISTS keeps it correct, just noisy if two workers boot at once — acceptable for current single-replica api); [4b] `DEFAULT_CONSOLE_DB_PATH` exported as public symbol (convenience for `app.py`, no spec mandate either way).

**Next-session opener (as written 2026-04-28 16:18)**

> "Continue Stage A on `feat/stage-a-foundation`: ship slice 2 — `argon2-cffi` dep + `src/api/auth/hashing.py` + `_seed_operator_zero()` wired into `init_db_console()` + tests for the seed under empty/missing/populated `console_password` states. Codex pre-commit review."
...
> **Resolved by slice 3g (d)(e) — commit `f3f95da`, 2026-04-28 night** — handler-level WS auth landed on both `/console/ws` and `/console/demo/ws/{scan_id}` per spec §5.2 Option 2; the data-leak surface is closed.
> **Resolved by slice 3g (f) — 2026-04-28 night** — `LegacyBasicAuthMiddleware` + `HEIMDALL_LEGACY_BASIC_AUTH` env-flip lever retired per §7.10 Option B; the env-flag bridge no longer exists. Rollback is `git revert <slice-3g-merge-sha>`. References to the flag in this entry remain as historical record of the pre-3g posture.

## 2026-04-28 (late evening) — Stage A slice 3f review-pass + slice 3g spec lock + audit refactor prep

**Decided**
- Slice 3f review-pass closed all six Codex findings (P1×2, P2×3, P3×1). Pivot history (Option 1 → Option 2 → Option 1) is preserved in the slice 3f decision-log entry as audit trail for future readers.
- Slice 3g spec locked at `docs/architecture/stage-a-slice-3g-spec.md` (commit `3ea2fcf`). All eleven §7 decisions resolved per Federico's review:
  - §7.10 Option B — retire `LegacyBasicAuthMiddleware` + `HEIMDALL_LEGACY_BASIC_AUTH` in slice 3g. Rollback is `git revert` only.
  - §7.11 Option B — hard-gate production deploy on slice 3g.5 (SPA Vitest harness + auth-flow tests). Slice 3g implementation may merge to `feat/stage-a-foundation` immediately; the bundle does NOT merge to `main` / push to `prod` until 3g.5 is green.
  - §7.1–§7.9 — see slice 3g decision-log entry for the locked outcomes table.
- Slice 3g implementation sequence (locked by reasoning, not by spec): audit refactor → (d)(e) WS handler auth + tests → (a)(b)(c) SPA work → (f) legacy retirement. Each commit leaves the branch in a working, tested state. Codex pass 1 on a (f)-first attempt flagged the branch would lose its only working browser-auth path; reordered before commit.
- Audit refactor shipped as commit `146ab53`: relocated `_maybe_write_disabled_operator_audit` from `src/api/auth/middleware.py` to `src/api/auth/audit.py` and renamed to public `maybe_write_disabled_operator_audit`. Pure refactor, no behavior change. Sets up §7.9 symmetry (HTTP middleware + future WS handler share one implementation). Dead imports cleaned (`datetime`/`UTC`/`Any`).
- Spec consistency fixes addressed all six Codex review findings:
  - P2 #3 (`/app` baseline contradiction): all "single-line revert" framing removed; §6 collapsed to a verification-only step.
  - P2 #4 (§7.9 partial-lock): summary table at top of §7 makes locked outcomes unambiguous in one glance.
  - P2 #5 (audit-writer transport): `_build_pseudo_request` adapter locked inside `src/api/console.py`; rejected widening `write_console_audit_row`.
  - P1 #1 (rollback under-specified) and P1 #2 (no SPA test coverage) drove §7.10/§7.11 to Option B.

**Rejected**
- Slice 3g implementation as one monolithic commit per spec §1's "ship in one commit" wording. Codex pass 1 confirmed the spec wording conflicts with CLAUDE.md "no large monolithic commits"; chose multi-commit sequencing where each commit leaves the branch tested + the bundle merges atomically to `main` only after slice 3g.5.
- Committing legacy retirement (f) first. Codex pass 1: removes the only working browser-auth path before SPA login form is built. Stashed for a final commit at the end of the slice.
- `/schedule` remote agent for slice 3g spec drafting. Reviewed against simple `Agent` tool — `Agent` dispatch is faster and synchronous for one-off tasks; `/schedule`'s value is recurring + time-shifted work, which this wasn't.

**Unresolved**
- Slice 3g remaining components NOT yet implemented:
  - (d)(e) WS handler auth on `/console/ws` + `/console/demo/ws/{scan_id}` + new `tests/test_console_ws_auth.py` (~240 LOC; backend, fully testable).
  - (a)(b)(c) SPA login form + whoami bootstrap + CSRF helper threading (~400 LOC across 4 new Svelte files + 3 edits; UNTESTABLE without Vitest harness — slice 3g.5 hard-gates).
  - (f) Legacy retirement: `stash@{0}` carries the work; pop + commit at the end of the implementation chain.
- Slice 3g.5 scope (locked at this session by reference): SPA Vitest harness + auth-flow tests covering login (200/401/429 paths), whoami bootstrap (200/204/409/401 branching), CSRF threading on mutations, mid-session 401 redirect, Retry-After countdown. Pattern reference: `apps/signup/` Vitest 21/21. Estimated ~400 LOC.
- CLAUDE.md "Status (2026-04-28)" updated this session to reflect slice 3g spec locked + audit refactor + 3g.5 model. Same staleness pattern flagged by the danger-zone hook across multiple recent entries — refresh again at next session if commits move state.
- Master spec `docs/architecture/stage-a-implementation-spec.md` will need a one-line §9.1 update when slice 3g (f) lands, noting legacy retired. Spec currently says "removed in the release after Stage A ships in prod" — slice 3g IS that release.
- Codex review on slice 3g spec doc commit (`3ea2fcf`) was deferred — the doc is not `src/**/*.py` and not `tests/**/*.py` so the precommit hook didn't fire. The spec content was Codex-reviewed iteratively during drafting (6 review passes); landing without one final post-edit pass is acceptable but flagged.

**Suggested opening prompt for next session:** "Resume slice 3g — start with (d)(e): handler-level WS auth in `src/api/console.py` for `/console/ws` and `/console/demo/ws/{scan_id}` plus the new `tests/test_console_ws_auth.py` (8 cases per master spec §8.2). Use the `_authenticate_ws(websocket)` helper extraction and the `_build_pseudo_request` adapter per slice 3g spec §4.2."

## 2026-04-28 (night) — Stage A slice 3g implementation complete (a–f)

**Context**
Slice 3g closes the auth-plane carve. Three sequenced commits on `feat/stage-a-foundation`, each Codex-cleared and individually testable. The bundle does NOT merge to `main` / push to `prod` until slice 3g.5 (SPA Vitest harness + auth-flow tests) lands per §7.11 Option B.

**Decided**
- **Commit `f3f95da` — slice 3g (d)(e): handler-level WS auth + tests.** `_authenticate_ws(websocket, *, audit_payload)` helper in `src/api/console.py` reads the `heimdall_session` cookie, hashes via SHA-256, validates against `console.db`, and either accepts + writes a `liveops.ws_connected` audit row inside the same `with conn:` (master §5.8 atomicity), or `accept()` + `close(4401)` per RFC 6455. `_WSRequestAdapter` / `_build_pseudo_request` is the duck-typed `Request` adapter per spec §4.2 so `write_console_audit_row` stays HTTP-Request-only. Both `/console/ws` and `/console/demo/ws/{scan_id}` call the helper (§7.7 Option A — same shape, parameterised audit payload). New `tests/test_console_ws_auth.py` ships 15 cases: 6 failure modes × 2 endpoints (no cookie, unknown cookie, revoked, idle-expired, absolute-expired, disabled-operator), 2 happy paths (one per endpoint) with audit-row + ping/pong assertions, and 1 middleware-bypass lock-in that wraps `_authenticate_ws` to prove the handler is reached even with no cookie. Each rejection test asserts WS close 4401, no `liveops.ws_connected` audit row, no delta on the four WS-handler pubsub channel subscriber counts (`PUBSUB NUMSUB` delta vs absolute — `console:logs` already has lifespan subscribers), and `sessions.last_seen_at` unchanged where applicable. Codex pre-commit: HOLD with 2 warnings (pubsub side-effects + last_seen_at coverage); clear after the rejection-path tests gained both invariants.
- **Commit `9f2f583` — slice 3g (f): legacy retirement per §7.10 Option B.** Deleted `LegacyBasicAuthMiddleware` class + `HEIMDALL_LEGACY_BASIC_AUTH` env-flip branch from `src/api/app.py`. Auth-router include simplified to unconditional. Dropped now-unused `base64` / `secrets` / `get_secret` imports. Test changes: removed `LegacyBasicAuthMiddleware` import + `test_legacy_flag_with_creds_mounts_legacy` + `test_legacy_flag_without_creds_falls_back_to_session` from `tests/test_session_auth.py`; rewrote `test_default_branch_mounts_session_auth` as a tighter `test_session_auth_middleware_mounted` (single Stage A branch, no env flag); dropped the `delenv("HEIMDALL_LEGACY_BASIC_AUTH", ...)` from the `configured_env` fixture. Stale-reference cleanup: `src/api/frontend/src/lib/api.js` `SESSION_REQUIRED_MESSAGE` no longer points operators at the retired flag; `tests/_console_auth_helpers.py` docstring no longer references the default-vs-legacy branch distinction; master spec §9.1 got a one-line retirement note above the original prose (preserved as rationale-of-record); `docs/decisions/log.md` slice 3f entry got forward cross-references marking the data-leak surface and env-flag bridge resolved by slice 3g (d)(e) and (f) respectively. Codex pre-commit: HOLD with 2 warnings + 2 info-level cross-reference gaps; clear after all four addressed. Test count: 154 → 152 (two legacy-mount tests deleted, default-branch test rewritten).
- **Commit `72d7386` — slice 3g (a)(b)(c): SPA login + whoami bootstrap + CSRF helper.** New `src/api/frontend/src/lib/auth.svelte.js` (~250 LOC) — `$state` runes hold `status / operator / csrfToken / retryAfter / error`; `bootstrap()` / `login()` / `logout()` / `getCsrfToken()` / `tickRateLimit()` / `handleSessionExpired()` are the public surface; module-level in-flight promise collapses concurrent `bootstrap()` callers (§7.6 race-mitigation); `getCsrfToken()` falls back to parsing `document.cookie['heimdall_csrf']` per §7.2 Option C (refresh-survivable). New `views/Login.svelte` (~190 LOC) wires the form to `login()` with `submitting / rate-limited / 401 / 503 / network` branches and a `$effect`-driven countdown for the rate-limited state. New `views/BootstrapEmpty.svelte` + `views/AllDisabled.svelte` are distinct splash components per §7.4 Option C + §7.5 Option B. Modified `App.svelte`: `onMount` calls `bootstrap()` before any other fetch; outer `{#if}` chain renders `loading` (boot splash) / `bootstrap-empty` / `all-disabled` / `authenticated` (Sidebar + Topbar + view router) / default `Login`; `$effect` on `auth.status` is the single WS startup site (§7.3 Option C — removed the duplicate `connect()` call from `login()` per Codex finding 1) and on the not-authenticated branch calls `disconnect()` before resetting Sidebar counters (per Codex finding 2 — prevents an open `/console/ws` from streaming behind the login screen after mid-session 401 / logout). Modified `lib/api.js`: `csrfHeaders()` threaded through `postJSON` / `saveSettings` / `sendCommand`; 401 path calls `handleSessionExpired()` (fire-and-forget) so the auth state machine flips to `'unauthenticated'` while the caller's failure UI renders the throw; `initialiseShell()`'s campaign-count fetch routed through `fetchCampaigns()` (per Codex finding 4 — raw fetch was bypassing the 401 handler). Modified `Topbar.svelte`: "Log out" button + `display_name` span, button only renders when `auth.status === 'authenticated'`. Vite build clean (467ms; 3 pre-existing warnings in unaffected files). Codex pre-commit (manual — JS/Svelte doesn't trigger the precommit hook but auth-critical surface warrants the pass): HOLD with 4 findings (2 high — duplicate `connect()` + missing `disconnect()`; 2 medium — bootstrap in-flight guard + raw `fetch`); clear after all four addressed.
- **Sequencing.** (d)(e) → (f) → (a)(b)(c) was the locked order from the slice 3g spec lock entry (2026-04-28 evening). Each commit leaves the branch tested + working; the bundle merges atomically to `main` only after slice 3g.5. Codex pass on a (f)-first attempt earlier had flagged "removes the only working browser-auth path before SPA login form is built" — the order chosen avoided that.
- **Rollback posture.** Single recovery path is `git revert <commit-sha>` (per §7.10 Option B, env-flip lever was retired with (f)). Commit-level revert is granular enough that any one of the three slice 3g commits can be reverted independently if needed before slice 3g.5 / merge to main.

**Rejected**
- Browser-eyeball QA at end-of-session (per `feedback_test_frontend_in_browser`). Federico operates the browser; the SPA pieces compile cleanly and pass static review but UI verification requires `make dev-up` + manual interaction. Slice 3g.5 shipping the Vitest harness will harden this loop with automated coverage; until then browser QA is Federico's call to schedule.
- Folding slice 3g.5 into this session. Spec §11 scopes 3g.5 as a separate slice (~400 LOC: Vitest harness configs + `auth.svelte.js` unit tests + Login.svelte component tests + bootstrap state-machine integration tests + CSRF threading assertion + mid-session 401 redirect test + Retry-After countdown test). Each is straightforward in isolation but needs its own spec lock and Federico's sign-off; not auto-mode work.

**Unresolved**
- **Slice 3g.5 spec.** Pattern reference for the Vitest harness: `apps/signup/` (`make signup-test`, 21/21 passing). Need a per-slice spec like the slice 3g one before implementation; suggested scope captured in slice 3g spec §11 + §8 of this entry. Production deploy of the slice 3g bundle is hard-gated on this per §7.11 Option B.
- **Browser-eyeball QA against `make dev-up`.** Confirm the four `auth.status` branches render correctly: clear console.db → boot reaches `BootstrapEmpty`; seed one operator → boot reaches `Login` → submit good creds → `Dashboard` loads + Topbar shows display_name + WS connects + retention actions still work; submit bad creds → invalid_credentials error; submit 5× bad → rate-limited countdown; click "Log out" → returns to `Login` + Topbar logout button hides. None of this is automated by the slice 3g tests; slice 3g.5 closes the loop.
- **CLAUDE.md "Status" paragraph.** Federico shrank it during this session (the verbose Stage A status block was collapsed to a one-line summary). Subsequent sessions should refresh from `git log --oneline feat/stage-a-foundation` + this decision-log entry rather than the prose paragraph for current state — per the recurring CLAUDE.md staleness pattern.
- **Stage A.5 sequence**, unchanged from prior entries: `Permission` enum + `require_permission` decorator + X-Request-ID middleware + `config_changes` triggers + `/console/config/history` after slice 3g.5 ships. Then V2 onboarding view.

**Suggested opening prompt for next session:** "Browser-eyeball QA the slice 3g SPA flow against `make dev-up`, then either lock the slice 3g.5 spec (Vitest harness + auth-flow tests) or merge `feat/stage-a-foundation` if 3g.5 is being deferred to a sibling branch. Three slice 3g commits to QA: `f3f95da` (WS handler auth) + `9f2f583` (legacy retirement) + `72d7386` (SPA login + whoami + CSRF)."

## 2026-04-29 — Stage A slice 3g.5 spec locked + harness configuration deviations

**Decided**
- Slice 3g.5 spec locked at `docs/architecture/stage-a-slice-3g-5-spec.md` (commit `ec04a9e`). All seven §7 decisions resolved per Federico (2026-04-29). §7.3 — the merge-blocking decision — landed on Option A + 1: separate `frontend` CI job parallel to the existing Python `test` job, with `package-lock.json` committed for both `src/api/frontend/` and `apps/signup/`. This makes the §7.11 Option B production-deploy hard-gate mechanical, not advisory; it also retroactively closes the existing CI gap where `apps/signup/` Vitest tests run locally but never gate PRs. Other six decisions locked at the architect's recommendation (test location → A; component approach → A `@testing-library/svelte`; countdown → B no fake-timers; bootstrap-collapse test → A required; Topbar test → A skip; aggregate `js-test` → B skip).
- **`vitest.config.js` deviations from spec §2.2 — implementation findings.** Spec §2.2 said the SPA's Vitest config mirrors `apps/signup/vitest.config.js` line-for-line. Two deviations were necessary in implementation:
  - **(a) `svelteTesting()` plugin from `@testing-library/svelte/vite`** — required for component tests (`Login.svelte`, `BootstrapEmpty.svelte`, `AllDisabled.svelte`) to render under jsdom + matchers + auto-cleanup. `apps/signup/` only tests plain ESM modules, so its harness can omit this plugin.
  - **(b) `preprocess: []` on `@sveltejs/vite-plugin-svelte`** — required to disable Vite 6's `preprocessCSS` chain. Under `vite@6.4.1` + `@sveltejs/vite-plugin-svelte@5.1.1`, the default style-block preprocessor calls `preprocessCSS` against an environment that doesn't exist in test-runner mode (`Cannot create proxy with a non-object as target or handler`). Heimdall's SPA uses plain CSS only — no SCSS / PostCSS plugins that would actually need preprocessing — so disabling the preprocessor chain has no cost and unblocks component tests.
  Both deviations are documented inline in `vitest.config.js`. The locked spec text was not retro-edited; this entry is the audit trail closing the gap (Codex flag, 2026-04-29 review pass).
- **`auth.svelte.js` `bootstrap()` refactored from `async` to sync-returning-promise.** The original slice 3g (a)(b)(c) implementation declared `bootstrap()` as `async`, which made the JavaScript runtime auto-wrap the in-flight promise on each call — defeating the `p1 === p2` identity contract that slice 3g.5 spec §5.2 test #1 locks. The refactor returns the in-flight promise directly via `.then(...).then(...)` cleanup chaining (instead of `async ... try/finally`). Behavioral effect: concurrent `bootstrap()` callers now receive the same promise object, not just the same fetch round trip. No regressions to existing callers (`App.svelte.onMount` fire-and-forgets; `handleSessionExpired()` `await`s inside its own `async` function; Codex confirmed both work with sync-returning-promise).
- **`bootstrap.test.js` test #3 (release-on-throw) covers the resolve-arm, not the reject-arm.** Codex flagged that `_bootstrapImpl()` catches the fetch-throw internally and translates it into `auth.error='network'`, so the `bootstrap()` wrapper's resolve handler runs (not the reject handler). The test name is preserved for spec §5.2 traceability; an inline comment in the test explains the actual coverage. The reject-arm cleanup callback in `auth.svelte.js` is dead under the current `_bootstrapImpl` contract and exists as defense-in-depth against a future refactor that lets a rejection escape.

**Rejected**
- Retro-editing the locked slice 3g.5 spec to reflect the §2.2 deviations. Spec is a frozen agreement at lock time; deviations during implementation are decision-log territory, not spec edits. Future slices' specs should reference this entry alongside the slice 3g.5 spec.
- Adding a `Topbar.svelte` component test (Decision §7.6 said skip; Codex pass confirmed `auth.test.js` test #20 already covers the post-logout state machine, which is the load-bearing part). Topbar is a one-line wire over the state machine.

**Unresolved**
- Browser-eyeball QA against `make dev-up` for the slice 3g + slice 3g.5 surface — Federico's call to schedule. Slice 3g.5's automated tests are the floor; eyeball QA is the ceiling for the four `auth.status` UI branches (boot splash → login → bootstrap-empty → all-disabled → authenticated shell with logout button + countdown render).
- CI gate verification — the new `frontend` job has only ever run on the Mac dev's local box (`make frontend-test`). Local pytest does not validate CI per `.claude/hooks/ci_config_reminder.py`; first push to `feat/stage-a-foundation` is the actual proof. Past failure mode (ruff added to CI but not to deps, 2026-03-XX) is not in play here — `npm ci` from the committed lockfile installs everything declared in `package.json` deterministically.
- Slice 3g bundle merge to `main` is now mechanically gated on this CI run going green. After CI green: merge to main → fast-forward prod → `HEIMDALL_APPROVED=1 git push origin prod` → SSH Pi5 → `heimdall-deploy`. The first Pi5 deploy of slice 3g + 3g.5 is the production cutover for the auth plane.

## 2026-04-29 (cont.) — Slice 3g.5 implementation + PR #51 + CI green on first run

**Decided**
- Slice 3g.5 implementation landed at commit `53944ba` on `feat/stage-a-foundation`. 7 components per spec §1: Vitest harness configs (a), `auth.test.js` 32 cases (b), `Login.test.js` 10 cases (c), `splash.test.js` 6 cases (d), `api.test.js` 12 integration cases (e), `bootstrap.test.js` 3 in-flight-collapse cases (f), CI integration (g). 63/63 tests pass locally in 1.16s.
- `auth.svelte.js` runtime change shipped: `bootstrap()` refactored from `async` to sync-returning-promise. The original `async` form auto-wrapped the in-flight promise on each call, defeating the `p1 === p2` identity contract spec §5.2 test #1 locks. Sync-returning-promise via `.then(...).then(...)` cleanup chain preserves identity. Codex confirmed no regressions to existing callers (`App.svelte.onMount` fire-and-forgets; `handleSessionExpired()` `await`s inside its own `async` function).
- PR #51 opened (`feat/stage-a-foundation` → `main`): the full Stage A bundle, slices 1 through 3g.5, 20 commits, +18,112 / −534 LOC. CI green on first run — `frontend` job 19s (operator-console + signup Vitest suites), `test` job 57s (Python). The §7.11 Option B production-deploy hard-gate is now mechanically enforced via the new `frontend` CI job.
- Pre-push validation discipline followed per Codex Area-6 flag: clean `npm ci` + `npm run test` + `npm run build` in `src/api/frontend/`, clean `npm ci` + `npm run test` in `apps/signup/`, 152/152 Python auth tests still green. The Vite production build (482ms) confirmed the `vitest.config.js` + `package.json` dev-dep additions don't regress the prod build.

**Rejected**
- Skipping the pre-push checklist. Codex's Area-6 flag was a real one — Vite production build with new dev-deps had real regression potential. Running it caught nothing but locked the no-regression evidence.
- Triggering CI via direct push to `feat/stage-a-foundation`. The repo's `ci.yml` only runs on `push: branches: [main]` or `pull_request: branches: [main]`, so the feature-branch push didn't fire CI; PR #51 was the only path to actually exercise the new gate.

**Unresolved**
- **Browser-eyeball QA against `make dev-up`** — Federico's manual verification of the four `auth.status` UI branches before merging PR #51 to `main`. Test plan in PR #51 body: empty `console.db` → BootstrapEmpty splash; seed operator → Login → submit good creds → Dashboard with display_name + WS connected; submit bad creds → invalid_credentials error; submit 5× bad → rate-limited countdown; click "Log out" → returns to Login. Plus DevTools-confirm `/console/ws` rejects unauthenticated WS upgrades with code 4401.
- **Node.js 20 deprecation warnings** on `actions/checkout@v4`, `actions/setup-node@v4`, `actions/setup-python@v5`. GitHub forces Node 24 default starting **2026-06-02**, removes Node 20 entirely **2026-09-16**. Worth a routine action-version bump before then. Candidate for a one-time `/schedule` agent ~2 weeks out.
- **Production cutover sequence**: PR #51 → merge to `main` → fast-forward `prod` → `HEIMDALL_APPROVED=1 git push origin prod` → SSH Pi5 → `heimdall-deploy`. The first Pi5 deploy of slice 3g + 3g.5 is the auth-plane cutover. SIRI approval is still the business gate, not a code gate; pilot launch remains blocked there.
- **CLAUDE.md slim went through Federico's hand-edit mid-session.** The committed slim file is correct; no follow-up needed, but worth noting for the audit trail that the rewrite was Claude-drafted, then user-edited mid-flow, then I committed.
- **Stage A.5 sequence** (next sprint after PR #51 merge + Pi5 cutover): `Permission` enum + `require_permission` decorator + X-Request-ID middleware + `config_changes` triggers + `/console/config/history`. Then V2 onboarding view.

**Suggested opening prompt for next session:** "Browser-QA the slice 3g + 3g.5 SPA against `make dev-up` per PR #51's test plan. If green: merge PR #51 → fast-forward prod → Pi5 deploy. If anything fails: revert at commit-level granularity (each of the four 3g/3g.5 commits is independently revertable)."

## 2026-04-29 (cont.) — Post-merge regression: `.dockerignore` allowlist gap on `console-db-schema.sql`

**Decided**
- PR #51 merged to `main` triggered the `publish-images.yml` workflow on `main` (it builds 5 ARM64 service images and pushes to GHCR for `heimdall-rollback` use). 4/5 services built clean — `twin`, `scheduler`, `delivery`, `worker`. The `api` build failed at `buildx`: `failed to compute cache key: failed to calculate checksum of ref ... "/docs/architecture/console-db-schema.sql": not found`. Run: https://github.com/stanz-stanz/heimdall/actions/runs/25082034684.
- Root cause: `.dockerignore` lines 29-33 (pre-fix) excluded everything under `docs/architecture/` and only allowlisted `client-db-schema.sql`. Slice 2 of Stage A introduced `console-db-schema.sql` (consumed by `init_db_console` and COPIED at `Dockerfile.api:57`) but never updated the `.dockerignore` allowlist. The local Dockerfile build succeeds because the local working tree has the file; CI's clean checkout + `.dockerignore` filter strips it from the build context, and `Dockerfile.api`'s `COPY docs/architecture/console-db-schema.sql ...` then fails with the not-found error.
- Fix landed in this commit: `!docs/architecture/console-db-schema.sql` added to `.dockerignore` immediately after the existing `!docs/architecture/client-db-schema.sql` allowlist line. Single-line change. Went direct to `main` per CLAUDE.md "Bug fixes → direct to main." No PR.
- The four other services (`twin`, `scheduler`, `delivery`, `worker`) built fine because their Dockerfiles don't COPY `console-db-schema.sql` — only `api` initialises `console.db`. So the regression scope is narrow: GHCR's `heimdall-api:main` + `heimdall-api:<sha>` tags are missing for `ee8662d`. Re-running `publish-images` after this fix lands on main produces the missing tags.
- This commit also lands the slice-3g.5 wrap-up entry above (it had been stashed during the post-merge rebase confusion when local `main` turned out to have a stale unpushed commit `d6df018` from a prior session). One commit, two changes: dockerignore fix + decision-log wrap-up. Audit-clean since both are pure docs/config — no Python diff, no precommit Codex hook gate.

**Rejected**
- Routing the dockerignore fix through a PR. CLAUDE.md branching rule: features → branch + PR; bug fixes → direct to main. This is the latter — a one-line allowlist add fixing a regression in a merged PR.
- Ignoring the `api` build failure on the assumption the next merge to main would rebuild from a fresh cache. Cache hits on `docker/build-push-action` only trigger if the layer's checksum can be computed; a missing file is a hard build error, not a cache miss. Every future `publish-images` run on main would have failed identically until the allowlist was patched.
- Leaving local `main`'s stale `d6df018` commit in place. It documents nothing not already in `docs/decisions/log.md` upstream and would confuse a future fast-forward push. To keep the destructive-git-guard hook happy without a `reset --hard`, this hotfix branched off `origin/main` directly; after push, local `main` will fast-forward cleanly to the new HEAD when the next `git pull` runs.

**Unresolved**
- `publish-images` re-trigger after this fix lands. No code path requires manual intervention — the workflow runs on every push to `main`, so this commit's push will fire it automatically.
- Pi5 deploy of `ee8662d` is now blocked on this fix landing AND `publish-images` going green. Without the `heimdall-api:main` tag, `heimdall-rollback` has no rollback target for the new SHA. Forward deploy via `heimdall-deploy` builds locally from source and is unaffected — but the rollback safety net is missing until the publish run succeeds.
- Why this slipped past slice 3g.5's CI gate: the `frontend` + `test` jobs in `ci.yml` run on PR-time on a fresh checkout but never invoke `docker build`. The `publish-images` workflow runs only after merge to main. There's no pre-merge gate that catches Dockerfile/dockerignore regressions; this is a known gap in the CI surface and an open candidate for a future "smoke build the api image on PR" step.

## 2026-04-30 — Auth-plane unblocked: action bumps, smoke gate, worker cross-compile, SPA bypass, rate-limit knob

**Decided**
- **PR #52 (`f1eac7b`)** — bumped `actions/checkout v4→v6`, `setup-node v4→v6`, `setup-python v5→v6`, `docker/build-push v5→v7`, `docker/setup-buildx v3→v4`, `docker/setup-qemu v3→v4`, `docker/login v3→v4`. Bumped `node-version: "20" → "24"` in the `frontend` job (Node 20 hits maintenance EOL 2026-04-30). Added `.github/workflows/docker-smoke.yml` — pre-merge matrix smoke build for all 5 services on `linux/amd64`, path-filtered to every COPY source across the 5 Dockerfiles (`infra/compose/**`, `.dockerignore`, `requirements.txt`, `src/**`, `config/**`, `scripts/**`, `tools/**`, `.claude/agents/**`, both `docs/architecture/*-db-schema.sql`), separate GHA cache scope (`smoke-<service>`), self-bootstrap path-filter entry. Catches the `console-db-schema.sql` allowlist class of regression at PR-time. Also added `scripts/ci/check_run_annotations.sh` — reusable warning-aware run-status checker (Federico's correction: "create a reusable script. Enough."). Saved as feedback memory.
- **PR #53 (`0a8a9ca`)** — Worker QEMU Priority 1: `go-builder` stage now runs on `$BUILDPLATFORM` natively and cross-compiles to `$TARGETPLATFORM`. `CGO_ENABLED=0` (alpine has no cross C toolchain; all 5 tools verified pure-Go). `GOBIN` dropped (Go refuses GOBIN with cross-compile, hard-coded check); per-platform output dir resolved by sweep step (`/go/bin/${TARGETOS}_${TARGETARCH}/` cross, `/go/bin/` native). Cold-cache `publish-images.yml` worker job: **56 min → 8 min** (run 25182054498). Beat the projected 25–30 min target — apt-get cache reuse contributed beyond the Go-compute win.
- **PR #54 (`9b5e443`)** — Slice 3g/3g.5 SPA-bypass chicken-and-egg fix. `SessionAuthMiddleware` was unconditionally 401-ing `/app/*`, blocking the SPA bundle from loading for unauth users — making BootstrapEmpty / Login / AllDisabled splashes unreachable. Codex confirmed spec contradiction shipped together: §69 + §101 require whoami-driven unauth UI on app mount, §357 + implementation-spec §1358 require `/app/*` protection; middleware enforced the second before the first could execute. Federico chose Option A (conservative least-privilege bypass): `_SPA_SHELL_PATHS = {"/app", "/app/", "/app/index.html"}` exact + `_SPA_ASSET_PREFIXES = ("/app/assets/",)` prefix + `_SPA_BYPASS_METHODS = {"GET", "HEAD"}`. Preserves `_AUTH_BYPASS_PATHS` exact-match invariant via parallel `_SPA_*` structures. 8 new tests in `tests/test_auth_middleware.py` + 1 fixup to `tests/test_session_auth.py::test_app_prefix_protected` (URL changed from `/app/` to `/app/whatever-else` post-bypass). Production smoke-check on rebuilt `heimdall-api:0a8a9ca-dirty` confirmed all 11 expected URLs match (5 bypass-200, 5 gated-401, 1 redirect-307).
- **PR #55** (in flight, branch `chore/auth-rate-limit-env-overrides`) — `HEIMDALL_AUTH_RATE_LIMIT_THRESHOLD` and `HEIMDALL_AUTH_RATE_LIMIT_WINDOW_SEC` env-vars override the spec defaults at module import. Production leaves both unset; defaults match the spec (5 fails, 15 min). Invalid (missing/empty/non-int/sub-1) values fall back to default — limiter must never run with zero/negative threshold/window. Narrow scope per Federico's instruction: "env override only, no behavior change beyond configurable defaults." 5 new helper-fn tests in `tests/test_auth_rate_limit.py`; existing `test_constants_match_spec` preserved unchanged. `.env.example` updated with commented-out opt-in entries mirroring the `CONSOLE_SESSION_*` pattern. Unblocks the slice 3g/3g.5 manual-QA walkthrough by allowing `THRESHOLD=2 WINDOW_SEC=10` for dev — sidesteps the 15-minute lockout the test plan would otherwise impose.
- **Worker QEMU optimization priority order locked** (saved as `project_worker_qemu_priorities.md`). Priority 1 ($BUILDPLATFORM cross-compile) — done. Priority 2 (stable base image for runtime-heavy layers — `apt-get`, `pip install`, `nuclei -update-templates`, CMSeeK clone) — deferred. Priority 3 (native arm64 runner) — infra/ops decision, deferred.
- **Self-review chain confirmed under load.** This session ran the full `discovery → domain agent → Codex → Federico` cycle three times: docker-expert for the Dockerfile.worker change (caught CGO recommendation), python-expert for the SPA-bypass middleware (clean first pass), and python-expert + codex-rescue for the rate-limit env-overrides. Codex caught no shippable findings on any of the three but verified the design choices, which is the contract.
- **Feedback memories saved this session:** `feedback_qa_time_knobs.md` (surface time-window constants and offer to dial them down before asking the user to wait through real-time delays), `feedback_question_style.md` (choice questions: each option one line, no recommendation paragraph, end on "Pick a or b" not "Which?"), `project_worker_qemu_priorities.md` (priority 1/2/3 lockdown).

**Rejected**
- Bumping `docker/*` actions in a separate PR after the initial `actions/*` v6 bump — Federico challenged the conservative scope, all bumps folded into PR #52.
- "Cross-compile saves 56 min → ~3 min" framing — Federico corrected: cross-compile only fixes the Go-compute portion. Runtime stage's `apt-get`/`pip install`/`nuclei -update-templates`/`git clone CMSeeK` still execute under QEMU because they target the real arch. Saved as project memory.
- Wider `/app/*` bypass (Option B) — Federico chose Option A conservative posture so future routes accidentally landing under `/app/api/*` cannot silently leak.
- Skipping pre-commit Codex review on the small `tests/test_session_auth.py` fixup — ran it anyway, verdict ship.
- Direct-to-main for the rate-limit env-override change — feature-class change touching `src/`, PR appropriate.
- One-off `gh api` annotation queries — Federico's correction "create a reusable script" turned the pattern into the committed `scripts/ci/check_run_annotations.sh`.

**Unresolved**
- **Browser-QA walkthrough of slice 3g/3g.5** — dev stack is up at http://localhost:8001/app/ (admin / devpassword), test plan handed over, not yet walked. Will run cleanly once PR #55 lands and the rate-limit knob can dial down to `THRESHOLD=2 WINDOW_SEC=10`.
- **Pi5 cutover for the auth-plane bundle** (PR #51 / `ee8662d`). Gated on browser-QA above. Sequence: fast-forward `prod` → `HEIMDALL_APPROVED=1 git push origin prod` → SSH Pi5 → `heimdall-deploy`. The first Pi5 deploy of slice 3g + 3g.5 is the auth-plane production cutover.
- **Stage A.5 sprint** — `Permission` enum + `require_permission` decorator + X-Request-ID middleware + `command_audit` + `config_changes` triggers + `/console/config/history`. Lands AFTER Pi5 cutover, BEFORE V2 onboarding.
- **PR #55 docker-smoke / CI** still running at session close. Will land green based on local 26/26 test pass and Codex SHIP verdict; merge when CI confirms.
- **Spec correction** to reconcile slice 3g spec §357 + implementation-spec §1358 (`/app/*` protected) with §69 + §101 (whoami drives unauth UI on mount). PR #54 fixed the implementation; the spec text still reads as if `/app/*` is fully gated. Worth a follow-up doc-only PR to mark the bypass list as authoritative.
- **Restart Claude Code harness** for the renamed `cloud-devsec` agent slug — Federico's.

**Suggested opening prompt for next session:** "Merge PR #55, restart api with `HEIMDALL_AUTH_RATE_LIMIT_THRESHOLD=2 HEIMDALL_AUTH_RATE_LIMIT_WINDOW_SEC=10`, walk the slice 3g/3g.5 browser test plan against http://localhost:8001/app/ (admin/devpassword); if green, fast-forward prod and Pi5-deploy."

---

## 2026-04-30 (cont.) — Production Readiness Gate locked + doc reconciliation + DRYRUN-CONSOLE retroactive approval

**Context.** Federico called for a complete product-plan review; tpmo synthesised a path-to-MVP across eight forks. Federico picked the **Production Readiness Gate** as the first work block: ship Stage A + A.5 to Pi5 with no documentation lies remaining, no auth-plane ambiguity, and a tested rollback. This entry locks the Gate scope, the five A.5 / Pi5 / rollback / Live-Twin / spec-format sub-decisions, retroactively approves the DRYRUN-CONSOLE seed plan that PR #50 shipped, and records the surgical doc edits that resolve the §357 / §1358 / Live Twin staleness.

**Decided**

- **Production Readiness Gate scope (this sprint).** Stage A.5 implementation (audit triggers, RBAC decorator, X-Request-ID middleware) + Pi5 auth-plane cutover + Stage A spec/doc reconciliation + DRYRUN-CONSOLE seed retroactive approval + Live Demo end-state verification + Pi5 deploy/rollback rehearsal. Definition of done: first Pi5 deploy of Stage A succeeds; login/session/audit path works in prod; rollback is tested not just documented; DRYRUN console seed is approved and usable; no auth-plane ambiguity remains in docs/specs. Gate unblocks everything downstream; the biggest "not actually ready" risk is removed from the rest of the path-to-MVP sequence. Federico's framing: "This removes the biggest 'not actually ready' risk from everything after it."
- **A.5 internal ordering — Option C.** Audit triggers + X-Request-ID parallel → RBAC decorator after. Federico's rationale: X-Request-ID is cheap and should land early, audit and request tracing belong together, RBAC follows once the request/audit path is stable.
- **Pi5 deploy ordering — Option B (bundled).** Stage A + A.5 ride one cutover. Federico's rationale: the point of A.5 is to make the first real prod auth cutover complete and auditable, not to create two separate risky cutovers.
- **Rollback rehearsal scope — Option C (image swap now, DB round-trip after A.5 migrations exist).** Image-tag swap rehearsal lands in this sprint; Alembic upgrade↔downgrade round-trip joins the rehearsal once A.5's migrations are written. Federico's rationale: rehearsing DB downgrade before A.5 migrations exist is low-signal theater.
- **briefing.md:334 — Option A (delete Live Twin sentence).** Federico's rationale: briefing copy must describe deployed reality, not internal intent. Live Twin animated mode was deleted in PR #43; the current `/app/#/demo` is theatrical replay only.
- **A.5 spec format — Option A (one bundled spec).** Federico's rationale: A.5 is still one coherent release slice. One spec, one implementation PR.
- **Architect dispatched** to draft `docs/architecture/stage-a-5-spec.md` covering all three components per the locked sub-decisions, ordering c, format a. Dependency graph (audit + X-Request-ID parallel → RBAC last) belongs in §3 of the spec. Spec is Codex-reviewed before Federico ratifies; implementation kicks off only after that.
- **DRYRUN-CONSOLE seed retroactive approval.** The seed-data plan was paused 2026-04-26 pending PR #47 merge; PR #50 (`feat/dryrun-console-seed`, merged) shipped the V1+V6 fixtures + seed scripts without a formal sign-off entry. The verification on 2026-04-28 caught two real bugs (day-0 `julianday` floor, day-7 same root cause); both fixed via the `[1, 1, ..., 7, 7, 7]` shift + range-tolerant verify script. The verify script is committed (`scripts/dev/verify_dev_console_seed.py`); the dev console renders V1 trial-expiring + V6 retention queue tabs correctly; lesson on reusable verification scripts already saved as `feedback_build_reusable_verify_scripts.md`. **Status: approved retroactively.** No further work required on the seed plan; it lives at PR #50 + the verify script.
- **Stage A doc reconciliation (this commit, direct-to-main per docs convention).** Three surgical edits resolve the §357 / §1358 / Live Twin staleness called out in the 2026-04-30 morning entry's Unresolved list:
  - `docs/briefing.md:334` — Live Twin sentence deleted.
  - `docs/architecture/stage-a-implementation-spec.md` — §8.2 `/app/...` test bullet split into `/app/api|ws/*` (auth required, all methods) + SPA shell+asset bypass (GET/HEAD only) + `_is_spa_public_asset` cross-prefix sanity. §9.1 closing line "the flag is removed in the release after Stage A ships in prod" rewritten as a historic-tense note. §9.1 retirement callout updated from a single-commit revert claim to an ordered two-commit chain (revert `9b5e443` first, then slice 3g) — PR #54's `_SPA_*` additions sit on top of slice 3g's `middleware.py` introduction, so reverse-applying slice 3g first leaves PR #54 hunks hanging on a deleted file (Codex verified via `git apply -R --check`).
  - `docs/architecture/stage-a-slice-3g-spec.md` — §6.1 postscript appended documenting PR #54's bypass list (`_SPA_SHELL_PATHS`, `_SPA_ASSET_PREFIXES`, `_SPA_BYPASS_METHODS`) as authoritative on top of `_PROTECTED_PREFIXES`. §6.2 added enumerating three locked passages (§5.2 test "unchanged" bullet, §6 verification step, §9.1 Lever 1 procedure) with corrected readings; references use heading anchors not line numbers. Original §6 prose preserved as locked rationale-of-record.

**Rejected**

- **Edit slice 3g spec §6 in place.** Slice 3g spec is LOCKED per its own header — amendments append as postscripts; the original §6 prose stays as rationale-of-record, matching the precedent set by the 2026-04-28 (recovered) entry that prefaced its own out-of-order placement.
- **Move Live Twin to a "Future / roadmap" section in briefing.md** (Option B on the fork). Live Twin was a deleted feature, not a deferred one — there's no roadmap signal worth preserving.
- **Bundle the A.5 spec into this commit.** Spec drafting is a separate track; lands as a separate commit once architect returns and Codex review passes.
- **Open Phase 0 as a feature branch + PR.** Doc-only commits go direct-to-main per the convention established by `b33575d` (the 2026-04-30 wrap-up commit). No `src/` or `tests/` Python diff; no Codex pre-commit gate fires from the hook. Codex review still runs on the diff because Federico's standing rule is "everything gets cross-checked by Codex" — see `feedback_self_review_chain.md`.
- **Bundle PR #55 wait into this entry.** PR #55 (rate-limit env-overrides) is a separate small PR already in flight; tracking lives in the 2026-04-30 morning entry's Unresolved list.

**Unresolved**

- **Architect drafting `docs/architecture/stage-a-5-spec.md`** (background). Awaiting return; Codex review follows; Federico ratifies before implementation kicks off.
- **PR #55 merge** (rate-limit env-overrides) — still in flight per 2026-04-30 morning entry. Unblocks browser-QA of the auth-plane bundle on dev.
- **Phase 1 (A.5 implementation).** Lands on `feat/stage-a-5-foundation` once spec is ratified. Sequencing: audit triggers + X-Request-ID parallel commits → RBAC decorator commit. Each Codex-reviewed pre-commit per the `precommit_codex_review_guard.py` hook.
- **Phase 2 (Pi5 rehearsal).** Browser-QA on dev → write `scripts/rollback_pi5.sh` (committed, not snippets — `feedback_build_reusable_verify_scripts`) → execute image-tag swap rehearsal on Pi5 (deploy N+1 → validate → rollback to N → validate → redeploy N+1) → run `scripts/validate_pi5.sh` at each step.
- **Phase 3 (first Pi5 cutover).** Bundled Stage A + A.5 deploy. Smoke checklist: login/logout, whoami four-state, sliding+absolute TTL, rate-limit 429, audit row appears for a config change, X-Request-ID present in logs. DoD signed into the decision log.
