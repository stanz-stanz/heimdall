...
Create the directories and initial files Valdí expects:

```
logs/valdi/                          # Forensic log entries (one .md per validation)
data/valdi/active_approvals.json     # Currently valid approval tokens
data/scan_types.json                 # Scan-type registry
data/clients/                        # Per-client authorisation files
data/compliance/                     # Per-target pre-scan check outputs
docs/legal/compliance-checklist.md   # Compliance checklist (from SKILL.md)
```

Initialize `active_approvals.json` and `scan_types.json` as empty structures:
...
Each function gets its own forensic log entry. Functions that pass are registered in `data/scan_types.json` with approval tokens. Functions that fail are logged as rejections with full reasoning and suggested remediation.
...
1. Claude Code writes or modifies a scanning function
2. Claude Code submits the function to Valdí for Gate 1 review
3. Valdí evaluates against `SCANNING_RULES.md` at the declared authorisation level
4. If REJECTED → Valdí writes forensic log with full reasoning. Claude Code must rewrite. No execution.
5. If APPROVED → Valdí writes forensic log, generates approval token, updates `active_approvals.json`, registers scan type in `data/scan_types.json`
6. Federico reviews the Valdí log entry and gives final go-ahead
7. Scanner executes, referencing the approval token

**Enforcement:** The scanning code must check `active_approvals.json` for a valid token matching its scan type before executing. If no valid token exists, it refuses to run and logs the failed attempt.

---

### 4. Implement the function hash check

Each approval token in `active_approvals.json` is linked to a `function_hash` (SHA-256 of the function source). Before execution, the scanner must:
...
2. The scan type is not added to `data/scan_types.json`
