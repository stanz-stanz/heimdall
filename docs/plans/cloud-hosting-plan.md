...
- MitID OIDC `redirect_uri` lands on the backend FastAPI at a clean path (`https://api.digitalvagt.dk/signup/mitid-callback`), not on the SvelteKit bundle. The `/api` prefix is a signup-host proxy convenience only — backend FastAPI router prefixes are clean (`/signup`, `/console`, `/webhooks`, `/health`), matching the existing `src/api/console.py:23` pattern. The slice-1 magic-link landing route uses `POST /signup/validate` (read-only); token consumption stays in the existing Telegram `/start` handler — see `docs/superpowers/specs/2026-04-25-svelte-signup-site-design.md`.
- SvelteKit → backend API path: **decided 2026-04-25** — Caddy reverse-proxy rule `signup.digitalvagt.dk/api/* → backend Tailscale IP` with `/api` prefix-strip. Backend FastAPI sees clean paths (`/signup/validate`, `/health`, `/console/*`, `/webhooks/*`, etc. — no `/api` prefix). SvelteKit bundle stays generic (relative `/api/*` fetches work in dev via Vite proxy and in prod via Caddy). Token consumption stays in the existing Telegram `/start` handler — `POST /signup/validate` is read-only.
